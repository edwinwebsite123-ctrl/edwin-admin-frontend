...
export default function ViewPlacementPosterPage() {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold">View Placement Poster</h1>
          <p className="text-sm text-gray-500">Placement poster details and information.</p>
        </div>
        <div className="flex items-center gap-2">
          <Link
            href={`/placement-posters/${placementPoster.id}/edit`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-2xl border border-black/5 bg-white p-6 shadow-sm">
            <h2 className="text-xl font-semibold text-gray-900 mb-4">Placement Poster</h2>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-500">Alt Text</label>
                <p className="text-sm text-gray-900">{placementPoster.alt}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="rounded-2xl border border-black/5 bg-white p-6 shadow-sm">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-sm text-gray-900">{new Date(placementPoster.created_at).toLocaleDateString()}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-500">Last Updated</label>
                <p className="text-sm text-gray-900">{new Date(placementPoster.updated_at).toLocaleDateString()}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-500">Poster ID</label>
                <p className="text-sm text-gray-900 font-mono">{placementPoster.id}</p>
              </div>
            </div>
          </div>
        </div>
