...
export default function EditTestimonialPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/testimonials/${params.id}/update/`, {
        method: "PUT",
        headers: {
      ...
      if (response.ok) {
        router.push(`/testimonials/${params.id}`);
      } else {
        const error = await response.json();
        alert(`Failed to update testimonial: ${JSON.stringify(error)}`);
      }
    } catch (error) {
    ...
    <div className="min-h-[calc(100vh-64px)] space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Update testimonial information and image.</p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-2xl border border-black/5 bg-white p-6 shadow-sm">
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          <div className="sm:col-span-2">
            <label htmlFor="text" className="block text-sm font-medium text-gray-700 mb-1">Testimonial Text *</label>
            <textarea
              id="text"
              name="text"
              rows={4}
        ...
        <div className="mt-6 flex items-center gap-2">
          ...
          <Link href={`/testimonials/${testimonial.id}`} className="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium hover:bg-gray-50">Cancel</Link>
        </div>
      </form>
    </div>
  );
