...
export default function ViewUGProgramPage() {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold">View UG Program</h1>
          <p className="text-sm text-gray-500">UG program details and information.</p>
        </div>
        <div className="flex items-center gap-2">
          <Link
            href={`/ug-programs/${ugProgram.id}/edit`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-2xl border border-black/5 bg-white p-6 shadow-sm">
            <h2 className="text-xl font-semibold text-gray-900 mb-4">{ugProgram.name} ({ugProgram.code})</h2>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
              <div>
                <label className="block text-sm font-medium text-gray-500">Subtitle</label>
                <p className="text-sm text-gray-900">{ugProgram.subtitle}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-500">Duration</label>
                <p className="text-sm text-gray-900">{ugProgram.duration}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-500">Students</label>
                <p className="text-sm text-gray-900">{ugProgram.students}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-500">Modules</label>
                <p className="text-sm text-gray-900">{ugProgram.modules}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-500">Rating</label>
                <p className="text-sm text-gray-900">{ugProgram.rating} ⭐</p>
              </div>
            </div>

            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-500 mb-2">Description</label>
              <p className="text-sm text-gray-900 whitespace-pre-wrap">{ugProgram.description}</p>
            </div>

            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-500 mb-2">Eligibility</label>
              <p className="text-sm text-gray-900 whitespace-pre-wrap">{ugProgram.eligibility}</p>
            </div>
          </div>

          {/* Specializations */}
          {ugProgram.specializations && ugProgram.specializations.length > 0 && (
            <div className="rounded-2xl border border-black/5 bg-white p-6 shadow-sm">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Specializations</h3>
              <div className="flex flex-wrap gap-2">
                {ugProgram.specializations.map((specialization, index) => (
          ...
          <div className="rounded-2xl border border-black/5 bg-white p-6 shadow-sm">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-sm text-gray-900">{new Date(ugProgram.created_at).toLocaleDateString()}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-500">Last Updated</label>
                <p className="text-sm text-gray-900">{new Date(ugProgram.updated_at).toLocaleDateString()}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-500">Program ID</label>
                <p className="text-sm text-gray-900 font-mono">{ugProgram.id}</p>
              </div>
            </div>
          </div>
        </div>
