...
export default function EditPGProgramPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/pg-programs/${params.id}/update/`, {
        method: "PUT",
        headers: {
      ...
      if (response.ok) {
        router.push(`/pg-programs/${params.id}`);
      } else {
        const error = await response.json();
        alert(`Failed to update PG program: ${JSON.stringify(error)}`);
      }
    } catch (error) {
      console.error("Error updating PG program:", error);
      alert("Error updating PG program");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold">Edit PG Program</h1>
          <p className="text-sm text-gray-500">Update PG program information and details.</p>
        </div>
        <div className="flex items-center gap-2">
          <Link
            href={`/pg-programs/${pgProgram.id}`}
      ...
      <form onSubmit={handleSubmit} className="rounded-2xl border border-black/5 bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 flex items-center gap-2">
          ...
          <Link href={`/pg-programs/${pgProgram.id}`} className="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium hover:bg-gray-50">Cancel</Link>
        </div>
      </form>
    </div>
  );
