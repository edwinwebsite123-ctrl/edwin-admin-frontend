...
export default function AddEdwinTalkPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/edwintalks/create/`, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        alert(`Failed to create Edwin talk: ${JSON.stringify(error)}`);
      }
    } catch (error) {
      console.error("Error creating Edwin talk:", error);
      alert("Error creating Edwin talk");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold">Add Edwin Talk</h1>
          <p className="text-sm text-gray-500">Create a new Edwin talk by filling the details below.</p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-2xl border border-black/5 bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 flex items-center gap-2">
          ...
          <Link href="/edwin-talks" className="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium hover:bg-gray-50">Cancel</Link>
        </div>
      </form>
    </div>
  );
