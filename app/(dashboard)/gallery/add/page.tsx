...
export default function AddGalleryItemPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/gallery/create/`, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        alert(`Failed to create gallery item: ${JSON.stringify(error)}`);
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold">Add Gallery Item</h1>
          <p className="text-sm text-gray-500">Create a new gallery item by filling the details below.</p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-2xl border border-black/5 bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 flex items-center gap-2">
          ...
          <Link href="/gallery" className="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium hover:bg-gray-50">Cancel</Link>
        </div>
      </form>
    </div>
  );
